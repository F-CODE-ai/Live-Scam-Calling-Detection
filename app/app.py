...
class ConnectionManager:
    ...
    async def connect(self, session_id: str, websocket: WebSocket):
        ...
        print(f"[CONNECT] session={session_id} now has {count} connection(s)")
    ...
    async def send_json(self, session_id: str, message: dict):
        ...
        receivers = self.active_connections.get(session_id, [])
        print(f"[BROADCAST] session={session_id} sending to {len(receivers)} receiver(s) -> risk_score={message.get('risk_score')}")
        for ws in list(receivers):
            try:
                await ws.send_text(json.dumps(message))
            except Exception as e:
                print(f"[BROADCAST ERROR] failed to send to one connection: {e}")
...
async def websocket_live_stream(websocket: WebSocket, session_id: str):
    ...
    try:
        while True:
            ...
            print(f"[RECEIVED] session={session_id} transcript='{transcript_text}'")
